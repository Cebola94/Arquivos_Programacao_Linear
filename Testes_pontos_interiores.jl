...
open("log_penalidade_caixa.txt", "w") do file
    ...
    for fun = 1:length(lista)
        ...
        MathProgBase.loadproblem!(m_internal, "MPS/"funcao)
        ...
        n_2 = length(c)
        c = [c;spzeros(n_1 - n_2)]
        x, λ, s, rb, rc, iter, ef, el_time = interior_point_method(A, b, c)
        str = @sprintf("%12s  %10.1e  %10.1e  %10.1e  %10.1e  %10.1e  %10.1e  %10.1e \n",funcao, dot(c,x), dot(λ,b), rb, rc, iter, ef, el_time)
        print(str)
        print(file,str)
    end
end
